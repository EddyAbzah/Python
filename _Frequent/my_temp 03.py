...
# ####   Files   #### #
try:
    path_log_folder = r"M:\Users\HW Infrastructure\PLC team\ARC\Temp-Eddy\Stage_2_Validation 15_01_24 19_19"
    file_scope = ""
    file_spi = "SPI Rec001 (InvCB=0,S01=0,S02=0).csv"
    SysCLK = 16667
    MainFreq = 511.4023
    print(f'''Python PLL input paths:
path_log_folder = {path_log_folder}
file_scope = {file_scope}
file_spi = {file_spi}
SysCLK = {SysCLK}[Hz]
MainFreq (PLL) = {MainFreq}[Hz]''')
except:
    print('exit(): error with one of the input paths')
    exit()

# ####   True   ###   False   #### #
scope_enable = file_scope is not None and file_scope != ''
output_plots_dpi = [True, 250]
adjust_scope_input_level_scope = [True, 1.03]
decimate_scope_input = [True, 3 if SysCLK == 16667 else 4]
...
if scope_enable:
    ...
    if output_plots_dpi[0]:
        if file_spi is not None or file_spi != '':
...
# if sys.gettrace() is not None:
    # plt.close()
try:
    dfs = pd.read_csv(path_log_folder + '\\' + file_spi).dropna(how='all', axis='columns')
    try:
        if plot_cut_before_spi[0]:
            indexes = dfs[dfs['Machine_State'].diff() == plot_cut_before_spi[1]].index.tolist()
        ...
        sum_of_dev = dfs['Phi_Sum_of_Dev'][plot_cut_after_spi[0]:plot_cut_after_spi[3]].reset_index(drop=True)
        abs_amps_sum = dfs['Abs_Amps'][plot_cut_after_spi[0]:plot_cut_after_spi[3]].reset_index(drop=True)
        phase_shift = max(sum_of_dev) if abs(max(sum_of_dev)) > abs(min(sum_of_dev)) else min(sum_of_dev)
        amplitude_shift = mean(abs_amps_sum[:plot_cut_after_scope[1]]) - mean(abs_amps_sum[plot_cut_after_scope[2]:])
        amplitude_ratio = (1 - (mean(abs_amps_sum[plot_cut_after_scope[2]:])) / mean(abs_amps_sum[:plot_cut_after_scope[1]])) * 100
        if output_plots_dpi[0]:
            ...
            axs[3 if scope_enable else 2].plot(dfs['SPI_Sample'], "c", label='SPI ADC In (RX4)')
            axs[3 if scope_enable else 2].plot(dfs['SOGI.Y1'], "r", label='SPI SOGI.Y1')
        print(f"SPI PLL\t{phase_shift:.2f}\t{amplitude_shift:.0f}\t{amplitude_ratio:.2f}")
    except Exception as error:
        print("An exception occurred:", error)
except:
    print(f'No SPI file found = {file_spi}')
