...
def plot_4_pane(subplot_title1, subplot_title2, subplot_title3, subplot_title4, series, key, Arc_power_diff,
                ...
                Amp_change_diff, ArcTH_power_diff_list, ArcTH_phase_shift_list,
                ArcTH_amp_ratio_list, ArcTH_amp_diff_list, time, bitmap_list, results_path):
    ...
    for i in range(len(Max_Phase_shift)):
        ...
        if Max_Phase_shift[i] > ArcTH_phase_shift_list[0] and Max_Phase_shift_bit[i] > 0:
    ...
    for i in range(len(Arc_power_diff)):
        ...
        if Arc_power_diff[i] > ArcTH_power_diff_list[0] and power_diff_bit[i] > 0:
    ...
    for i in range(len(Amp_change_diff)):
        ...
        if Amp_change_diff[i] > ArcTH_amp_diff_list[0] and amplitude_bit[i] > 0:
    ...
    "plot Th"
    fig_18xF.add_trace(
        go.Scattergl(y=ArcTH_power_diff_list, x=time, name=key + " " + subplot_title1 + ' th'),
        row=1, col=1)
    fig_18xF.add_trace(
        go.Scattergl(y=ArcTH_phase_shift_list, x=time, name=key + " " + subplot_title2 + ' th'),
        row=2, col=1)
    fig_18xF.add_trace(
        go.Scattergl(y=ArcTH_amp_ratio_list, x=time, name=key + " " + subplot_title3 + ' th'),
        row=3, col=1)
    fig_18xF.add_trace(
        go.Scattergl(y=ArcTH_amp_diff_list, x=time, name=key + " " + subplot_title4 + ' th'),
        row=4, col=1)
...
def plot_4_histogram(subplot_title1, subplot_title2, subplot_title3, subplot_title4, series, key, Arc_power_diff,
                     ...
                     ArcTH_power_diff_list, ArcTH_phase_shift_list,
                     ArcTH_amp_ratio_list, ArcTH_amp_diff_list, time, bitmap_list, results_path):
    bigger_then_th_power_diff = round(
        len([item for item in Arc_power_diff if item > ArcTH_power_diff_list[0]]) / len(Arc_power_diff) * 100, 2)
    bigger_then_th_Max_Phase_shift = round(
        len([item for item in Max_Phase_shift if item > ArcTH_phase_shift_list[0]]) / len(Max_Phase_shift) * 100, 2)
    bigger_then_th_Amp_change_percentage = round(
        len([item for item in Amp_change_percentage if item > ArcTH_amp_ratio_list[0]]) / len(
            Amp_change_percentage) * 100, 2)
    bigger_then_th_Amp_change_diff = round(
        len([item for item in Amp_change_diff if item > ArcTH_amp_diff_list[0]]) / len(Amp_change_diff) * 100, 2)
    ...
    fig.add_trace(
        ...
        row=1, col=1)
    fig.add_trace(
        go.Histogram(x=ArcTH_power_diff_list[:10], nbinsx=500, name=subplot_title1 + " Th"),
        row=1, col=1)
    fig.add_trace(
        go.Histogram(x=Max_Phase_shift, nbinsx=500, name=subplot_title2),
        row=1, col=2)
    fig.add_trace(
        go.Histogram(x=ArcTH_phase_shift_list[:10], nbinsx=500, name=subplot_title2 + " Th"),
        row=1, col=2)
    fig.add_trace(
        go.Histogram(x=Amp_change_percentage, nbinsx=500, name=subplot_title3),
        row=2, col=1)
    fig.add_trace(
        go.Histogram(x=ArcTH_amp_ratio_list[:10], nbinsx=500, name=subplot_title3 + " Th"),
        row=2, col=1)
    fig.add_trace(
        go.Histogram(x=Amp_change_diff, nbinsx=500, name=subplot_title4),
        row=2, col=2)
    fig.add_trace(
        go.Histogram(x=ArcTH_amp_diff_list[:10], nbinsx=500, name=subplot_title4 + " Th"),
        row=2, col=2)
...
def plot_4_pane_D(subplot_title1, subplot_title2, subplot_title3, subplot_title4, series, key, Arc_power_diff,
    ...
    "plot Th"
    fig_18xF.add_trace(
        go.Scatter(y=ArcTH_power_diff_list, x=time, name=key + " " + subplot_title1 + ' th'),
        row=1, col=1)
    fig_18xF.add_trace(
        go.Scatter(y=ArcTH_phase_shift_list, x=time, name=key + " " + subplot_title2 + ' th'),
        row=2, col=1)
    fig_18xF.add_trace(
        go.Scatter(y=ArcTH_amp_ratio_list, x=time, name=key + " " + subplot_title3 + ' th'),
        row=3, col=1)
    fig_18xF.add_trace(
        go.Scatter(y=ArcTH_amp_diff_list, x=time, name=key + " " + subplot_title4 + ' th'),
        row=4, col=1)
    config = {'scrollZoom': True}
...
def All_data_stage_1_histograms(df_18xF, ArcTH_power_diff, ArcTH_phase_shift, ArcTH_amp_ratio,
                                ArcTH_amp_diff, bitmap_list, results_path, date):
    bigger_then_th_power_diff = round(
        len([item for item in df_18xF['Arc_power_diff'] if item > ArcTH_power_diff]) / len(
            df_18xF['Arc_power_diff']) * 100, 2)
    bigger_then_th_Max_Phase_shift = round(
        len([item for item in df_18xF['Max_Phase_shift'] if item > ArcTH_phase_shift]) / len(
            df_18xF['Max_Phase_shift']) * 100, 2)
    bigger_then_th_Amp_change_percentage = round(
        len([item for item in df_18xF['Amp_change_percentage'] if item > ArcTH_amp_ratio]) / len(
            df_18xF['Amp_change_percentage']) * 100, 2)
    bigger_then_th_Amp_change_diff = round(
        len([item for item in df_18xF['Amp_change_diff'] if item > ArcTH_amp_diff]) / len(
    ...
    fig.add_trace(
        ...
        row=1, col=1)
    fig.add_trace(
        go.Histogram(x=[ArcTH_power_diff] * 1000, nbinsx=1000, name='Arc power diff' + " TH"),
        row=1, col=1)
    fig.add_trace(
        go.Histogram(x=df_18xF['Max_Phase_shift'], nbinsx=1000, name="Max_Phase_shift"),
        row=1, col=2)
    fig.add_trace(
        go.Histogram(x=[ArcTH_phase_shift] * 1000, nbinsx=1000, name="Max_Phase_shift" + " TH"),
        row=1, col=2)
    fig.add_trace(
        go.Histogram(x=df_18xF['Amp_change_percentage'], nbinsx=1000, name="Amp_change_percentage"),
        row=2, col=1)
    fig.add_trace(
        go.Histogram(x=[ArcTH_amp_ratio] * 1000, nbinsx=1000, name="Amp_change_percentage" + " TH"),
        row=2, col=1)
    fig.add_trace(
        go.Histogram(x=df_18xF['Amp_change_diff'], nbinsx=1000, name="Amp_change_diff"),
        row=2, col=2)
    fig.add_trace(
        go.Histogram(x=[ArcTH_amp_diff] * 1000, nbinsx=1000, name="Amp_change_diff" + " TH"),
        row=2, col=2)
...
if __name__ == '__main__':
    ...
    input_file_path = r'C:\Users\eddy.a\Documents\Python Scripts\False Alarm Test Analysis\Files\_Input.csv'
    results_path = r'C:\Users\eddy.a\Documents\Python Scripts\False Alarm Test Analysis\Files'

    # change "ArcTH" params before start
    ArcTH_power_diff = 1000
    ArcTH_phase_shift = 4
    ArcTH_amp_diff = 800
    ArcTH_amp_ratio = 10
    ...
    for sn in df_18xF_dict_sorted_by_len:
        ...
        bitmap_list = []
        time = []
        ArcTH_power_diff_list = []
        ArcTH_phase_shift_list = []
        ArcTH_amp_diff_list = []
        ArcTH_amp_ratio_list = []
        temp = []
        for key, values in sn.items():  ##for each serial....
            ...
            for event in range(len(values)):  ##for each event that happend in this serial
                ...
                ArcTH_power_diff_list.append(ArcTH_power_diff)
                ArcTH_phase_shift_list.append(ArcTH_phase_shift)
                ArcTH_amp_diff_list.append(ArcTH_amp_diff)
                ArcTH_amp_ratio_list.append(ArcTH_amp_ratio)
    ...
    All_data_stage_1_histograms(df_18xF, ArcTH_power_diff, ArcTH_phase_shift, ArcTH_amp_ratio,
                                ArcTH_amp_diff, bitmap_list, results_path, date)
