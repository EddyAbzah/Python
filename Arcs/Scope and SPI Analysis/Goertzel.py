...
class GoertzelSampleBySample:
    ...
    def process_sample(self, samp):
        ...
        S = self.koef * self.vn1 - self.vn2 + samp
        self.vn2 = self.vn1
        self.vn1 = S
        ...
        if self.cnt_samples == self.nsamp:
            ...
            return Goertzel.VOLT(self.koef, self.vn1, self.vn2, self.nsamp)
        return None
...
def GoertzelSampleBySample_func(df, detect_freq, After_ds_Fs, N):
    samp = GoertzelSampleBySample(detect_freq, After_ds_Fs, N)
    ...
    if (comparison):
        for idx, sample in enumerate(df):
            ...
            if temp is not None:
                dft_out_goertzel_1_TONE.append(temp)
                samp.reset()

    return dft_out_goertzel_1_TONE
