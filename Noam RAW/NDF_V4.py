...
def scpectrogram_plot(Z, t, f, max_plot_res, fmax, fmin, t_start, t_stop, plot_on, results_path, meas_sig, file=''):
    """
    ...
    layout = go.Layout(title=f'Spectrogram [dBm] of {file}: ' + results_path.split('\\')[-1] if meas_sig == '' else meas_sig,
...
def spectrogram_plot_for_streamlit(Z, t, f, max_plot_res, fmax, fmin, meas_sig, file=''):
    """
        Function generates spectrogram plot
    ...
    layout = go.Layout(title=f'Spectrogram [dBm] of {file}: {meas_sig}',
                       yaxis=dict(title='Frequency [Hz]'),  # x-axis label
                       xaxis=dict(title='Time [sec]'),  # y-axis label
                       )
