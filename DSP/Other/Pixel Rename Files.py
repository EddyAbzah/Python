...
def get_renaming_scheme(filename):
    ...
    if match:
        ...
        return {"original_name": filename, "new_name": new_filename, "date": original_utc_datetime}


def check_for_duplicates(file_data, destination_directory):
...
if __name__ == '__main__':
    ...
    for index, filename in enumerate(files):
        if index + 1 < index_start or index - index_start + 2 > index_count:
            continue
        file_info = get_renaming_scheme(filename)
        file_data.append(file_info)

    if check_for_duplicates(file_data, directory):
