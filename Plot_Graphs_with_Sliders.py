...
def add_data(df, title, index=0.1, true_if_matrix=False, title_matrix='', delete_indexes=None, state_to_keep=1):
    global plots
    global plot_titles
    global plot_matrix
    global plot_counter
    if index != 0.1:
        ...
        if delete_indexes != None:
            df = [a if b == state_to_keep else None for a, b in zip(df, delete_indexes)]
        plots.append([df, title])
        plot_titles.append(title)
        plot_matrix.append(False)
        plot_counter += 1
