...
class F_Event:
    ...
    def __init__(self,date,time,code,Arc_power_diff,Bitmap,Max_Phase_shift,Amp_change_diff,Amp_change_percentage,full_time):
        self.date = date
        self.time = time
        self.code = code
...
class D_Event:
    ...
    def __init__(self,date,time,code,Mngr_pwr,ArcMaxEnergy,ArcMaxCurrentDrop,ArcAverageIac,ArcminEnergy,full_time):
        self.date = date
        self.time = time
        self.code = code
...
def plot_4_pane(subplot_title1,subplot_title2,subplot_title3,subplot_title4,series,key,Arc_power_diff,Max_Phase_shift,Amp_change_percentage,
    ...
    fig_18xF.add_trace(
        go.Scattergl(y=Arc_power_diff, x=time,
                   ...
                   mode="markers",marker=marker_power_dict,customdata=list(zip(bitmap_decimal_bit, power_diff_bit)),hovertemplate="<br>".join(["Time: %{x}","value: %{y}","decimal bitmap;power diff bit: %{customdata}" ])
        ),row=1, col=1)
    fig_18xF.add_trace(
        go.Scattergl(y=Max_Phase_shift, x=time,
                   name=str(round(series.loc[key], 2)) + "% " + key+ " " + subplot_title2,
                   mode="markers", marker=marker_Phase_shift_dict,customdata=list(zip(bitmap_decimal_bit, Max_Phase_shift_bit)),
                   hovertemplate="<br>".join(["Time: %{x}","value: %{y}","decimal bitmap;Max Phase shiftbit: %{customdata}"])
                   ),
        row=2, col=1)
    fig_18xF.add_trace(
    ...
    fig_18xF.add_trace(
        go.Scattergl(y=Amp_change_diff, x=time, name=str(round(series.loc[key], 2)) + "% " + key + " "+ subplot_title4,
                   ...
                   hovertemplate="<br>".join(["Time: %{x}","value: %{y}","decimal bitmap;amplitude_bit: %{customdata}"])),
        row=4, col=1)
    ...
    plotly.offline.plot( fig_18xF, config = config, filename =results_path + '\\'+ key + ' Arc params '+str(round(series.loc[key], 2))+"%"  +date+ '.html')
def plot_4_histogram(subplot_title1,subplot_title2,subplot_title3,subplot_title4,series,key,Arc_power_diff,Max_Phase_shift,Amp_change_percentage, Amp_change_diff,
    ...
    plotly.offline.plot( fig, config = config, filename =results_path + '\\'+ key+ ' Arc histogram Stage1, '+str(round(series.loc[key], 2))+"%" +date+'.html')
    print("hey")
def plot_4_pane_D(subplot_title1,subplot_title2,subplot_title3,subplot_title4,series,key,Arc_power_diff,Max_Phase_shift,Amp_change_percentage,
                Amp_change_diff,Arc_power_diff_th_list,MaxPhaseshift_th_list,
                Amp_change_percentage_th_list,Amp_change_diff_th_list,time,bitmap_list,results_path ):
    plot_titles = [subplot_title1, subplot_title2, subplot_title3,subplot_title4]
    all_specs = np.array([[{"secondary_y": True}] for x in range(len(plot_titles))])
...
def plot_4_histogram_D(subplot_title1,subplot_title2,subplot_title3,subplot_title4,series,key,
    ...
    plotly.offline.plot( fig, config = config, filename =results_path + '\\'+ key+ ' Arc histogram Stage2, '+str(round(series.loc[key], 2))+"%," +date+'.html')
    print("hey")
def plot_avg_events_per_day(count_df,count_df_stage2,results_path,date,Stage_1_Real_Events,Stage_2_Real_Events):
    ...
    plot_titles = ["Average Stage 1 events per day","Real Stage 1 events per day", "Average Stage 2 events per day","Real Stage 2 events per day"]
    ...
    plotly.offline.plot(fig_histogram_per_day,config=config,filename=results_path+'\\'+'Events per day(Real+Avg)'+date+'.html')
...
def summary_pie_plot(count_df,count_df_stage2,results_path,date):
    ...
    fig_pie.add_trace(go.Table(header=dict(values=['Total inverters','Updated to AFCI2.2','Number of inverters' + '\n'+' that pass stage 1','number of ' + '\n'+ 'stage 1 events']),
                                   cells=dict(values=[number_of_invertes_in_fleet,number_of_UPDATED_AFCI,total_pass_stage1_inverters,[sum_of_pass_stage1_events]])),
        row=1, col=1)
    fig_pie.add_trace(go.Table(header=dict(values=['Total inverters','Updated to AFCI2.2','Number of inverters' + '\n'+' that pass stage 2','number of ' + '\n'+ 'stage 2 events']),
                                   cells=dict(values=[number_of_invertes_in_fleet,number_of_UPDATED_AFCI,total_pass_stage2_inverters,[sum_of_pass_stage2_events]])),
        row=2, col=1)
    config = {'scrollZoom': True}
...
def All_data_stage_1_histograms(df_18xF,Arc_power_diff_th,MaxPhaseshift_th,Amp_change_percentage_th,Amp_change_diff_th,bitmap_list,results_path,date):
    bigger_then_th_power_diff = round(
        len([item for item in df_18xF['Arc_power_diff'] if item > Arc_power_diff_th]) / len(df_18xF['Arc_power_diff']) * 100, 2)
    bigger_then_th_Max_Phase_shift = round(
        len([item for item in df_18xF['Max_Phase_shift'] if item > MaxPhaseshift_th]) / len(df_18xF['Max_Phase_shift']) * 100, 2)
    bigger_then_th_Amp_change_percentage = round(
    ...
    bigger_then_th_Amp_change_diff = round(
        len([item for item in df_18xF['Amp_change_diff'] if item > Amp_change_diff_th]) / len(df_18xF['Amp_change_diff']) * 100, 2)
    ...
    plotly.offline.plot(fig, config=config, filename=results_path+'\\'+'All data stage 1 histograms '  +date+'.html')
...
def All_data_stage_2_histograms(df_18xD,results_path,date):
    ...
    plotly.offline.plot(fig, config=config, filename=results_path+'\\'+'All data stage 2 histograms '  +date+'.html')
...
if __name__ == '__main__':
    ...
    "Read data"
    csv_path = r"C:\Users\eddy.a\Documents\Python Scripts\False Alarm Test Analysis\13.1.21-3.2.21.csv"
    csv_path_filtered= r'C:\Users\eddy.a\Documents\Python Scripts\False Alarm Test Analysis\13.1.21-3.2.21_FIL.csv'
    results_path= r"C:\Users\eddy.a\Documents\Python Scripts\False Alarm Test Analysis\MANA"

    if not os.path.exists(results_path):
        os.makedirs(results_path)
    txt = results_path + '\\'
    signal_df = pd.read_csv(csv_path,low_memory=False)
    signal_df = signal_df.loc[lambda x: x['PowerBox'] == 'Log']
    ...
    signal_df.to_csv(csv_path_filtered, index=False,header=False,columns=['TELEM DATE','TELEM TIME','SN','V IN','I IN','V OUT','ACC CURRENT','TELEM SEC','TELEM POWER'])
    del signal_df
    signal_df = pd.read_csv(csv_path_filtered)
    ...
    "Renaming df"
    df_18xF.rename(columns={'Param1Float': 'Arc_power_diff', 'Param2Float': 'Bitmap', 'Param3Float': 'Max_Phase_shift','Param4Float':'Amp_change_diff','Param5Float':'Amp_change_percentage' ,
                            'TELEM DATE': 'TELEM_DATE','TELEM TIME': 'TELEM_TIME','Error Code':'Error_Code'}, inplace = True)

    df_18xD.rename(columns={'Param1Float': 'Mngr_pwr', 'Param2Float': 'ArcMaxEnergy', 'Param3Float': 'ArcMaxCurrentDrop',
    ...
    for row in df_18xF.iterrows():
        new_event = F_Event(row[1].TELEM_DATE, row[1].TELEM_TIME, row[1].Error_Code, row[1].Arc_power_diff, row[1].Bitmap, row[1].Max_Phase_shift, row[1].Amp_change_diff, row[1].Amp_change_percentage,row[1].Time)
    ...
    for row in df_18xD.iterrows():
        new_event = D_Event(row[1].TELEM_DATE, row[1].TELEM_TIME, row[1].Error_Code, row[1].Mngr_pwr, row[1].ArcMaxEnergy, row[1].ArcMaxCurrentDrop, row[1].ArcAverageIac, row[1].ArcminEnergy,row[1].Time)
    ...
    for sn in df_18xF_dict_sorted_by_len:
        ...
        for key, values in sn.items(): ##for each serial....
            ...
            plot_4_pane('Arc_power_diff', 'Max_Phase_shift', 'Amp_change_percentage', 'Amp_change_diff',
                    count_df['count_stage_1%'], key,Arc_power_diff,Max_Phase_shift,Amp_change_percentage, Amp_change_diff,Arc_power_diff_th_list, MaxPhaseshift_th_list,
                    Amp_change_percentage_th_list, Amp_change_diff_th_list, time,bitmap_list,results_path)
    ...
    fig.update_layout(title='Stage 1->Real Events  events per day'+"<br>"+'Maximum number of events per day is '+str(max( DF_hist.value_counts())))
    ...
    fig.update_layout(title='Stage 2->Real Events  events per day'+"<br>"+'Maximum number of events per day is '+str(max( DF_hist.value_counts())))
    fig.update_xaxes(title_text='Real events per day')
    fig.update_yaxes(title_text='inv count')
