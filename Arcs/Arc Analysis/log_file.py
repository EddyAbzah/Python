...
def get_files(folder_path, string_filter, dropna=True, skip_if_not_pwr=True):
...
def get_logs_all(log_file):
    ...
    all_logs['log_state'] = log_file[9].to_numpy().tolist()
    all_logs['log_energy_state'] = log_file[6].to_numpy().tolist()
    all_logs['log_current_state'] = log_file[7].to_numpy().tolist()
    all_logs['log_voltage_state'] = log_file[8].to_numpy().tolist()
    return all_logs
...
def avg_with_overlap(df, jump, overlap_before, overlap_after):
    """Array to average by sample_ratio (example: if old_sample_rate=50Hz & new_sample_rate=50kHz → sample_ratio=1000"""
    after_filter = []
    for i in range(jump, len(df), jump):
        sliced = df[i - overlap_before:i + 1 + overlap_after]
        after_filter.append(pd.DataFrame.mean(sliced))
    return convert_to_df(after_filter)
