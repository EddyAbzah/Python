...
# ## same lists as above:
sync_energy_detection = 0   # -12 is best
sync_current_detection = 0   # -13 is best
...
def energy_rise_algo2(log_energy, window_size=20, filter_size=15, over_th_limit=12):
    if print_method_calls:
        print(f'Getting the Energy Rise (algo 2); initialize_empty_lists = {initialize_empty_lists}')
    if initialize_empty_lists:
        energy_th_list = []
    else:
        energy_th_list = [0 for x in range(window_size + filter_size)]
...
def current_drop_algo2(log_current, window_size=20, filter_size=15, over_th_limit=12):
    if print_method_calls:
        print(f'Getting the Current Drop (algo 2); initialize_empty_lists = {initialize_empty_lists}')
    if initialize_empty_lists:
        current_th_list = []
    else:
        current_th_list = [0 for x in range(window_size + filter_size)]
    for sample_index in range(window_size + filter_size, len(log_current)):
        avg_filter_window = log_current[(sample_index - window_size - filter_size):sample_index - window_size].mean()
        sorted_window = list(log_current[(sample_index - window_size):sample_index].sort_values(ascending=False))
        if avg_filter_window - sorted_window[over_th_limit - 1] > 0:
            current_th_list.append(avg_filter_window - sorted_window[over_th_limit - 1])
        else:
            current_th_list.append(0)
    if sync_current_detection != 0:
        if sync_current_detection > 0:
            current_th_list[0:0] = [0 for i in range(sync_current_detection)]
            del current_th_list[-sync_energy_detection:]
        else:
            sync_current_detection_absolute = abs(sync_current_detection)
            del current_th_list[:sync_current_detection_absolute]
            current_th_list.extend([0 for i in range(sync_current_detection_absolute)])
...
def voltage_drop_algo2(log_voltage, window_size=20, filter_size=15, over_th_limit=12):
    if print_method_calls:
        print(f'Getting the Current Drop (algo 2); initialize_empty_lists = {initialize_empty_lists}')
    if initialize_empty_lists:
        voltage_th_list = []
        ...
        voltage_th_list = [0 for x in range(window_size + filter_size)]
    for sample_index in range(window_size + filter_size, len(log_voltage)):
        avg_filter_window = log_voltage[(sample_index - window_size - filter_size):sample_index - window_size].mean()
        sorted_window = list(log_voltage[(sample_index - window_size):sample_index].sort_values(ascending=False))
        if avg_filter_window - sorted_window[over_th_limit - 1] > 0:
            voltage_th_list.append(avg_filter_window - sorted_window[over_th_limit - 1])
        else:
            voltage_th_list.append(0)
    if sync_voltage_detection != 0:
        if sync_voltage_detection > 0:
            voltage_th_list[0:0] = [0 for i in range(sync_voltage_detection)]
            del voltage_th_list[-sync_energy_detection:]
        else:
            sync_voltage_detection_absolute = abs(sync_voltage_detection)
            del voltage_th_list[:sync_voltage_detection_absolute]
            voltage_th_list.extend([0 for i in range(sync_voltage_detection_absolute)])
...
def voltage_drop_algo2_Ks(log_voltage, th, window_size=20, filter_size=15):
    if print_method_calls:
        print(f'Getting the Current Drop (algo 2); initialize_empty_lists = {initialize_empty_lists}')
    if initialize_empty_lists:
        voltage_th_list = []
    ...
    for sample_index in range(window_size + filter_size, len(log_voltage)):
        avg_filter_window = log_voltage[(sample_index - window_size - filter_size):sample_index - window_size].mean()
        window = list(log_voltage[(sample_index - window_size):sample_index].sort_values(ascending=False))
        counter = 0
        for sample_window in window:
            if avg_filter_window - th > sample_window:
                counter += 1
        voltage_th_list.append(counter)
    if sync_voltage_detection != 0:
        if sync_voltage_detection > 0:
            voltage_th_list[0:0] = [0 for i in range(sync_voltage_detection)]
            del voltage_th_list[-sync_energy_detection:]
...
def energy_rise_algo_old(log_energy, th, window_size=20, filter_size=15, over_th_limit=12):
    if print_method_calls:
        print(f'Getting the Energy Rise (algo 2); initialize_empty_lists = {initialize_empty_lists}')
    if initialize_empty_lists:
        energy_th_list = []
    ...
    for sample_index in range(window_size + filter_size, len(log_energy)):
        min_filter_window = min(log_energy[(sample_index - window_size - filter_size):sample_index - window_size])
        filter_window = log_energy[(sample_index - window_size):sample_index]
        Counter = 0
        for value_filter_window in filter_window:
            if value_filter_window > th + min_filter_window:
                Counter += 1
        energy_th_list.append(Counter)
    if sync_energy_detection != 0:
        if sync_energy_detection > 0:
            energy_th_list[0:0] = [0 for i in range(sync_energy_detection)]
            del energy_th_list[-sync_energy_detection:]
...
def energy_rise_floor(log_energy, window_size=20, filter_size=15, over_th_limit=12):
    if print_method_calls:
        print(f'Getting the Energy Rise (algo 2); initialize_empty_lists = {initialize_empty_lists}')
    if initialize_empty_lists:
        energy_th_list = []
        ...
        energy_th_list = [0 for x in range(window_size + filter_size)]
    for sample_index in range(window_size + filter_size, len(log_energy)):
        energy_th_list.append(min(log_energy[(sample_index - window_size - filter_size):sample_index - window_size]))
    return energy_th_list


def Ks(log_voltage, th, window_size=20, filter_size=15):
    voltage_th_list = [0 for x in range(window_size + filter_size)]
    for sample_index in range(window_size + filter_size, len(log_voltage)):
        avg_filter_window = log_voltage[(sample_index - window_size - filter_size):sample_index - window_size].mean()
        window = log_voltage[(sample_index - window_size):sample_index]
        counter = 0
        for sample_window in window:
            if avg_filter_window - th > sample_window:
                counter += 1
