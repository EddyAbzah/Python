...
output_name_list = ['SPI Sample Least Squares N=100', 'Least Squares Self Test 300 samples']
...
def Ls_phase_est(folder, files):
    ...
    for file_index, file in enumerate(files):
        ...
        if self_test_freq__phase__amp_shifs[0]:
            ...
            if special_spi_cut[0] and any(s in file.lower() for s in special_spi_cut[1].keys()):
                indexes = special_spi_cut[1][file[file.lower().find('rec') + 3:file.lower().find('rec') + 6]]
                dfs = dfs[indexes[0]:indexes[1]].reset_index(drop=True)
        ...
        abs_phase = np.arctan2(np.imag(phase_jump), np.real(phase_jump)) * 180 / np.pi
        ...
        fig.add_trace(go.Scattergl(x=phase_time, y=abs_phase, name='Absolute Phase [°]', mode="lines", visible=False, line=dict(color="green"), showlegend=True), row=2, col=1)
