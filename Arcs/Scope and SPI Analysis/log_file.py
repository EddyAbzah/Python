...
def get_files(folder_path, string_filter, dropna=True):
    ...
    for file_number, file in enumerate(list_of_files):
        ...
        if 'scope' in file_name or 'spi' in file_name or 'mngr' in file_name or 'zes' in file_name:
            continue
        ...
        if dropna:
            log_file_all.append(pd.read_csv(StringIO(file_after_filter_2), sep=log_delimitation_type, header=None,
                                            error_bad_lines=False, warn_bad_lines=True, skipinitialspace=True).dropna())
        else:
            log_file_all.append(pd.read_csv(StringIO(file_after_filter_2), sep=log_delimitation_type, header=None,
                                            error_bad_lines=False, warn_bad_lines=True, skipinitialspace=True))
...
def alpha_beta_filter(df, alpha, return_df=False):
    """Filters a Data Frame with an Alpha / Beta Filter"""
    after_filter = [df[0]]
    for index in range(1, len(df)):
        after_filter.append(alpha * df[index] + (1 - alpha) * after_filter[index - 1])
    if return_df:
        after_filter = convert_to_df(after_filter)
    return after_filter


def avg_no_overlap(df, old_sample_rate, new_sample_rate):
    sample_ratio = int(old_sample_rate / new_sample_rate)
    avg_no_overlap(df, sample_ratio)


def avg_no_overlap(df, sample_ratio):
    """Array to average by sample_ratio (example: if old_sample_rate=50Hz & new_sample_rate=50kHz → sample_ratio=1000"""
    after_filter = []
    for i in range(0, len(df), sample_ratio):
        sliced = df[i:sample_ratio + i]
        after_filter.append(pd.DataFrame.mean(sliced))
    return convert_to_df(after_filter)


def avg_no_overlap_list(df, old_sample_rate, new_sample_rate, convert_to_pd_df=True):
    """Array to average by sample_ratio (example: if old_sample_rate=50Hz & new_sample_rate=50kHz → sample_ratio=1000"""
    after_filter = []
    sample_ratio = int(old_sample_rate / new_sample_rate)
    for i in range(0, len(df), sample_ratio):
        sliced = df[i:sample_ratio + i]
        after_filter.append(float(np.mean(sliced)))
    if convert_to_pd_df:
        return convert_to_df(after_filter)
    else:
        return after_filter
...
def send_email(sender, receivers, message):
    sender = 'noam.dahan1@solaredge.com'
    receivers = ['eddy.a@solaredge.com']
